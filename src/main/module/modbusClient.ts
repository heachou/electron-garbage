...
export class ModbusClient {
  ...
  public async readRegisters(
    params: ReadRequestParams & { port: string; bufferReturn?: boolean }
    ...
    try {
      ...
      return params.bufferReturn ? response.buffer : response.data // 返回解析后的数字数组
    } catch (error) {
  ...
  public async writeMultipleRegisters(
    params: WriteMultipleRequestParams & { port: string }
  ): Promise<{ address: number; quantity: number }> {
    const client = this.modbusClients.get(params.port)
    if (!client ||!client.isOpen) {
      throw new Error(`Port ${params.port} is not open or modbus client not available.`)
    }
    try {
      client.setID(params.deviceAddress)
      client.setTimeout(3000)
      const response = await client.writeRegisters(params.startAddress, params.values)
      return response as unknown as Promise<{ address: number; quantity: number }> // 返回库的响应对象
    } catch (error) {
      console.error(`Error writing multiple registers on port ${params.port}:`, error)
      throw error
    }
  }
